...
This is **not byte-identical to the historical validated archive**. Private registry names and historical fixture identifiers are replaced with unenrolled placeholders. Existing image/runtime-hash constraints remain intentionally restrictive. The generated archive is HOLD; obtaining an archive does not authorize execution or attest that historical CPU images equal a newly built image. Deployment enrollment must bind the new solver/runtime/CPU/OCI identities, reconcile the isolated table prefix with the selected table and supply private credentials/watchdogs. The historical production host is not changed by this build.
