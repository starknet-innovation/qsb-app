...
export async function saveObservation(store:Store,scope:Row,intent:Row,value:unknown){
 ...
 const journal={pk:intent.pk,sk:`OBS_RECEIPT#${intent.sk}:${digest}`,version:1,owner:intent.owner,revision:intent.revision,provider:intent.provider,observation};
 try{await store.atomicPut([{row:journal},...parts.map(row=>({row}))]);}catch(error){const saved=await store.get(journal.pk,journal.sk);if(!saved||saved.owner!==intent.owner||saved.revision!==intent.revision||saved.provider!==intent.provider||JSON.stringify(saved.observation)!==JSON.stringify(observation))throw error;await readObservation(store,{...intent,observation});}
...
/** Recover immutable terminal evidence even if its later scope/intent CAS lost. */
export async function savedTerminal(store:Store,intent:Row){
 if(intent.observation)return readObservation(store,intent);
 const receipts=await store.list(intent.pk,`OBS_RECEIPT#${intent.sk}:`);
 if(receipts.length===0)return undefined;
 if(receipts.length!==1)throw Error('Conflicting terminal observations require reconciliation');
 const receipt=receipts[0];
 if(receipt.owner!==intent.owner||receipt.revision!==intent.revision||receipt.provider!==intent.provider)throw Error('Saved terminal ownership mismatch');
 return readObservation(store,{...intent,observation:receipt.observation});
}
