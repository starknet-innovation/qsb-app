...
export function loadPredecessor(store:CensusStore,key:string,directory:string,seen=new Set<string>(),retainedResources=false,expectedConfigHash?:string):any{
 ...
 const file=(name:string)=>{const p=join(directory,name);if(lstatSync(p).isSymbolicLink())throw Error('Linked predecessor artifact');const bytes=readFileSync(p);if(name==='config.json'&&expectedConfigHash&&h(bytes)!==expectedConfigHash)throw Error('Exact external configuration bytes changed');return bytes.toString('utf8');};
