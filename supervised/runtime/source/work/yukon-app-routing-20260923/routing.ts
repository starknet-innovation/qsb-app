...
import {assertDepositCudaProgram,assertSearchUsesDepositProgram,requireEnrolledCudaProgram,WATCHED_YUKON_SUBSET} from '../../outputs/qsb-vault/src/lib/cuda-program';
...
type Vault={id:string;network?:string;config:string;scriptHex:string;scriptHash:string;publicStateJson:string;configuration?:unknown;funding?:{txid:string;vout:number;value:string};cudaProgram?:{id:string;kernelCommit:string;releaseHash:string}};
...
export function routeStoredJob(job:{id:string;owner:string;vaultId:string;manifest:unknown;solver?:any;execution?:unknown},vault:Vault){
 if(job.execution===undefined){const d=job.solver?assertSolverPin(job.solver,vault):solverRelease('qsb-config-a-ranked-v2-2791ed0');if(vault.configuration&&!job.solver)throw Error('SolverPinRequired');assertSearchUsesDepositProgram(d,vault.cudaProgram);return {target:'historical-step-functions' as const,descriptor:d};}
