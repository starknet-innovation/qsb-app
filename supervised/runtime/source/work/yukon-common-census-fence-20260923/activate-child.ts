...
export async function activateSubsetChild(store:Store,parent:string,child:string,owner:string,revision:number,budget:{maxSubmissions:number;deadlineMs:number},observe:()=>Promise<Observation>,cpu:Cpu){
 ...
 const ctx=JSON.parse(p.publicContext as string);if(ctx.network!=='regtest')throw Error('Only isolated regtest activation');
 ...
 await store.atomicPut([{row:{...p,version:p.version+1,phase:'subset_running'},expected:p.version},{row:{...c,version:c.version+1,phase:'searching',dispatchAuthorized:true,budget:{...budget,claimed:0,maxConcurrent:1},activation:execution},expected:c.version}]);
