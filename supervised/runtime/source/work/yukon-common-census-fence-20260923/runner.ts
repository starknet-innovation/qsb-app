...
import {drive} from '../yukon-indexed-controller-20260923/driver';
const ADAPTER='f82d12eb4ef56db8d1a896572d6397d3cd9c167830c48abfa9fa184eb883274e';
export type Config={parent:string;scope:string;owner:string;revision:number;endpoint:string;deadlineMs:number;maxSubmissions:number;maxOperations:number;pollIntervalMs:number};
...
export async function operate(store:Store,config:Config,provider:ProviderIO,observe:()=>Promise<Observation>,operation:string,intent:string|undefined,cpu:Cpu,signal?:AbortSignal){
 ...
 const budget=child.budget as {deadlineMs:number;maxSubmissions:number;maxConcurrent:number}|undefined;
 if(!budget||!Number.isSafeInteger(c.deadlineMs)||!Number.isSafeInteger(c.maxSubmissions)||c.maxSubmissions<1||budget.deadlineMs!==c.deadlineMs||budget.maxSubmissions!==c.maxSubmissions||budget.maxConcurrent!==1)throw Error('Composed budget mismatch');
 ...
 class AbortAwareProvider extends ProviderIO {
  ...
  override send(endpoint:string,event:unknown){if(signal?.aborted)throw Error('Operation aborted before paid request');return provider.send(endpoint,event);}
  override read(endpoint:string,id:string){return provider.read(endpoint,id);}
 }
 const guardedObserve=async()=>{if(signal?.aborted)throw Error('Operation aborted');const observation=await observe();if(signal?.aborted)throw Error('Operation aborted');return observation;};
 ...
 if(operation==='run')return drive(controller,publicEventFactory(JSON.parse(child.template),cpu),{maxOperations:c.maxOperations,deadlineMs:c.deadlineMs,pollIntervalMs:c.pollIntervalMs,signal});
