import {adoptExhaustedContext} from './cycle-adoption';
import {validateLineage} from './lineage';
import {adoptSettled} from './adoption';
import type {EvidenceReaders} from '../yukon-signing-export-20260923/publication';
import {terminalParentWrites} from '../yukon-adaptive-flow-20260923/completion';
import {pinReady} from './pin-ready';
import {prepareExport} from './export-core';
...
export type Blueprint={format:'qsb-common-lifetime-v1';network:'regtest'|'mainnet';runId:string;pin:PinConfig;subset:SubsetConfig};
...
export class CommonLifetime {
 ...
 constructor(private store:CensusStore,blueprint:Blueprint,private pinProvider:PinProvider,subsetProvider:SubsetProvider,private observePin:()=>Promise<any>,private observeSubset:()=>Promise<any>,private drain:()=>Promise<Drain>,private emit:(event:any)=>void,preserve:(event:any)=>Promise<void>,private runCpu:OwnedRunner,private preserveCpu:(event:any)=>Promise<void>,private disablePin?:()=>Promise<any>){
  ...
  if(!['regtest','mainnet'].includes(blueprint.network)||blueprint.format!=='qsb-common-lifetime-v1'||p.parent!==s.parent||p.owner!==s.owner||p.revision!==s.revision||p.endpoint===s.endpoint||s.scope===p.parent||typeof runCpu!=='function')throw Error('Invalid common blueprint');
 ...
 async adopt(old:Blueprint,shutdown:any,readers:EvidenceReaders){
  if(this.registered||this.busy||this.stopping)throw Error('Continuation already active');this.busy=true;this.enrollmentSettled=new Promise(resolve=>{this.settleEnrollment=resolve;});
  try{const receipt=await adoptSettled(this.store,old,this.blueprint,shutdown,readers);this.registered=true;this.settleEnrollment?.();if(this.stopping)throw Error('Stopped during adoption');
   const o=await this.owner();this.stage=o.lifecycle==='subset'?'subset':'pin';
   if(this.stage==='pin')await this.pin.adoptBootstrap();else await this.subset.adoptLinkedLifetime();
   return receipt;
  }finally{this.settleEnrollment?.();this.busy=false;}
 }
 async cycle(old:Blueprint,shutdown:any,readers:EvidenceReaders){
  if(this.registered||this.busy||this.stopping)throw Error('Context already active');this.busy=true;this.enrollmentSettled=new Promise(resolve=>{this.settleEnrollment=resolve;});
  try{const receipt=await adoptExhaustedContext(this.store,old,this.blueprint,shutdown,readers);this.registered=true;this.settleEnrollment?.();if(this.stopping)throw Error('Stopped during context adoption');this.stage='pin';await this.pin.adoptBootstrap();return receipt;}finally{this.settleEnrollment?.();this.busy=false;}
 }
 async initialize(context:any){
  context=structuredClone(context);if(context?.network!==this.blueprint.network)throw Error('Enrolled chain mismatch');
  ...
  try{
   ...
   await this.store.atomicPut([{row:{pk:'VALIDATION#'+p.parent,sk:'SCOPE',version:1,owner:p.owner,revision:p.revision,endpoint:p.endpoint,network:this.blueprint.network,stage:'pinning',phase:'bootstrap',identitySchema:'provider-index-v1',supervisedRun:this.runPk,supervisedConfigHash:this.configHash}},{row:{pk:this.runPk,sk:'OWNER',version:1,configHash:this.configHash,parent:p.parent,owner:p.owner,revision:p.revision,network:this.blueprint.network,status:'operating',stage:'pinning',activeOperation:key,lifecycle:'bootstrap'}},{row:{pk:this.runPk,sk:key,version:1,status:'running',kind:'bootstrap',configHash:this.configHash}}]);this.registered=true;this.settleEnrollment?.();
   ...
   await new PinControllerV3(bootstrap,p.parent,p.owner,p.revision,this.pinProvider,this.observePin,this.cpu).initialize(context,{maxSubmissions:p.maxSubmissions,deadlineMs:p.deadlineMs,submissionCutoffMs:p.submissionCutoffMs});await this.pin.adoptBootstrap();
  }finally{this.settleEnrollment?.();this.busy=false;}
 }
 private async cpuCall(kind:CpuOperation,payload:any){
  payload=structuredClone(payload);if(payload.network!==undefined&&payload.network!==this.blueprint.network)throw Error('CPU chain differs');payload.network=this.blueprint.network;const inputHash=hash(payload);
  ...
  const operationKey=owner.activeOperation,scopeBinding={runPk:this.runPk,configHash:this.configHash,operationKey,network:this.blueprint.network};let registered:any,completed:any;
  ...
  const pv=(out.ownedVerification as any).publicStateValidation,ctx=kind.startsWith('subset-')?payload.context:{publicStateJson:payload.publicStateJson,manifest:payload.manifest};
  if(!pv||pv.validatorSourceHash!=='26fe26a7e0e11bb6f71483a51d75b69076d86ea3a15bd2f01869156e5c1c1b36'||pv.publicStateHash!==createHash('sha256').update(ctx.publicStateJson).digest('hex')||pv.contextHash!==createHash('sha256').update(canonical(ctx).replace(/[\u007f-\uffff]/g,c=>'\\u'+c.charCodeAt(0).toString(16).padStart(4,'0'))).digest('hex')||pv.privateFieldsAccepted!==false||pv.consensusVerified!==false)throw Error('Checked public state receipt differs');
  if(this.stopping)throw Error('Stopped during CPU operation');return out.result;
 }
 private async owner(){
  const o=await this.store.get(this.runPk,'OWNER');if(!this.registered||!o||o.network!==this.blueprint.network||o.configHash!==this.configHash||o.owner!==this.blueprint.pin.owner||o.revision!==this.blueprint.pin.revision)throw Error('Lifetime ownership mismatch');await validateLineage(this.store,o,this.runPk);return o;
 }
 async operation(op:string,intent?:string){
  ...
  try{
   const parent=await this.store.get('VALIDATION#'+this.blueprint.pin.parent,'SCOPE');if(!parent||parent.network!==this.blueprint.network||parent.supervisedRun!==this.runPk||parent.supervisedConfigHash!==this.configHash||parent.dispatchClosed||parent.identityConflict)throw Error('Common parent closed');
   ...
   const terminal=this.stage==='subset'?await terminalParentWrites(this.store,this.blueprint.pin.parent,this.blueprint.subset.scope,this.blueprint.pin.owner,this.blueprint.pin.revision,this.runPk,this.configHash):[];
   await this.store.atomicPut([...terminal,{row:{...current,version:current.version+1,status:this.stopping?'dispatch_stopped':'owned',activeOperation:null},expected:current.version},{row:{...operation,version:operation.version+1,status:'completed'},expected:operation.version}]);return result;
 ...
 async extend(winner:string){
  if(Date.now()>=this.blueprint.subset.submissionCutoffMs)throw Error("Subset submission cutoff");
  ...
  try{
   await this.pin.transportDrained();const owner=await this.owner(),ready=await pinReady(this.store,p.parent,p.owner,p.revision,winner),parent=ready.scope;
   if(parent.network!==this.blueprint.network||parent.supervisedRun!==this.runPk||parent.supervisedConfigHash!==this.configHash||owner.activeOperation||!this.disablePin)throw Error('Owned handoff actuator unavailable');
   await this.store.atomicPut([{row:{...parent,version:parent.version+1,handoffDispatchClosed:true},expected:parent.version},{row:{...owner,version:owner.version+1,status:'operating',activeOperation:opKey,lifecycle:'extending'},expected:owner.version},{row:{pk:this.runPk,sk:opKey,version:1,status:'running',kind:'pin-publication-child-bootstrap',configHash:this.configHash,winner,terminalInventoryHash:hash(ready)}}]);
   if(this.stopping)throw Error('Stopped before capacity transition');
   const disabled=await this.disablePin();
   if(disabled?.format!=='qsb-pin-disabled-v1'||disabled.endpoint!==p.endpoint||disabled.workersMin!==0||disabled.workersMax!==0||disabled.queued!==0||disabled.inProgress!==0)throw Error('Capacity transition unconfirmed');
   const currentOwner=await this.owner(),currentParent=await this.store.get(pk,'SCOPE'),operation=await this.store.get(this.runPk,opKey);
   if(this.stopping||currentOwner.activeOperation!==opKey||!currentParent||currentParent.version!==parent.version+1||currentParent.identityConflict||currentParent.phase!=='pinning_draining'||!operation||operation.status!=='running')throw Error('Handoff changed during disable');
   await this.store.atomicPut([{row:{...currentOwner,version:currentOwner.version+1},expected:currentOwner.version},{row:{...currentParent,version:currentParent.version+1},expected:currentParent.version},{row:{...operation,version:operation.version+1,disableReceipt:disabled},expected:operation.version}]);
   await this.observeDrain();
   ...
   await activateSubsetChild(guarded,p.parent,s.scope,p.owner,p.revision,{maxSubmissions:s.maxSubmissions,deadlineMs:s.deadlineMs,submissionCutoffMs:s.submissionCutoffMs},async()=>{await this.observeDrain();if(this.stopping)throw Error('Stopped during extension');return this.observeSubset();},this.cpu);
 ...
 /** Verify and retain exact public signing material before stop changes solved phase metadata. */
 async prepareSolvedState(request:any){
  request=structuredClone(request);
  const owner=await this.owner();if(this.busy||this.stopping||owner.status!=='owned'||owner.activeOperation)throw Error('Export preparation needs idle active common owner');
  const parent=await this.store.get('VALIDATION#'+this.blueprint.pin.parent,'SCOPE'),child=await this.store.get('VALIDATION#'+this.blueprint.subset.scope,'SCOPE');
  if(!parent||!child||parent.supervisedRun!==this.runPk||child.supervisedRun!==this.runPk||parent.supervisedConfigHash!==this.configHash||child.supervisedConfigHash!==this.configHash||parent.phase!=='subset_prepared'||child.phase!=='awaiting_authorization'||parent.identityConflict||child.identityConflict||parent.dispatchClosed||child.dispatchClosed)throw Error('No unmodified completed solution context');
  const key='OPERATION#'+randomUUID();this.busy=true;
  try{
   await this.store.atomicPut([{row:{...owner,version:owner.version+1,status:'operating',activeOperation:key},expected:owner.version},{row:{...parent,version:parent.version+1},expected:parent.version},{row:{...child,version:child.version+1},expected:child.version},{row:{pk:this.runPk,sk:key,version:1,status:'running',kind:'signing-export-preparation',configHash:this.configHash}}]);
   const prepared=await prepareExport(this.store,{parent:this.blueprint.pin.parent,child:this.blueprint.subset.scope,owner:this.blueprint.pin.owner,revision:this.blueprint.pin.revision},request,{pin:this.cpu.pinHandoff,subset:this.cpu.subset});
   const currentOwner=await this.owner(),op=await this.store.get(this.runPk,key),p=await this.store.get(parent.pk,'SCOPE'),c=await this.store.get(child.pk,'SCOPE');
   if(this.stopping||!op||op.status!=='running'||currentOwner.activeOperation!==key||JSON.stringify(p)!==JSON.stringify(prepared.snapshot.parent)||JSON.stringify(c)!==JSON.stringify(prepared.snapshot.child))throw Error('Export preparation scope changed');
   const immutable={format:'qsb-mainnet-solved-preparation-record-v1',runPk:this.runPk,configHash:this.configHash,operationKey:key,requestId:request.id,prepared,preparedHash:fingerprint(prepared)};
   // Bounded immutable public chunks avoid DynamoDB item size assumptions. Manifest is published only with the final fence.
   const encoded=JSON.stringify(immutable),parts=Array.from({length:Math.ceil(encoded.length/48000)},(_,i)=>encoded.slice(i*48000,(i+1)*48000));
   for(let i=0;i<parts.length;i++)await this.store.put({pk:child.pk,sk:'EXPORT_PREP_CHUNK#'+request.id+':'+key+':'+i,version:1,text:parts[i],sha256:createHash('sha256').update(parts[i]).digest('hex')});
   await this.store.atomicPut([{row:{...currentOwner,version:currentOwner.version+1,status:'owned',activeOperation:null},expected:currentOwner.version},{row:{...op,version:op.version+1,status:'completed',preparationHash:immutable.preparedHash},expected:op.version},{row:{...p!,version:p!.version+1,signingPreparation:request.id},expected:p!.version},{row:{...c!,version:c!.version+1,signingPreparation:request.id},expected:c!.version},{row:{pk:child.pk,sk:'EXPORT_PREP#'+request.id,version:1,format:immutable.format,runPk:this.runPk,configHash:this.configHash,requestId:request.id,operationKey:key,chunkCount:parts.length,encodedHash:createHash('sha256').update(encoded).digest('hex'),preparedHash:immutable.preparedHash,parentPreparedVersion:p!.version+1,childPreparedVersion:c!.version+1}}]);
   return {requestId:request.id,preparedHash:immutable.preparedHash,bundleSha256:prepared.receipt.bundleSha256,signingAuthorized:false};
  }finally{this.busy=false;}
 }
 ...
 private async stopAfterEnrollment(){
  ...
  if(!this.registered){const o=await this.store.get(this.runPk,'OWNER');if(!o||o.network!==this.blueprint.network||o.configHash!==this.configHash||o.owner!==this.blueprint.pin.owner||o.revision!==this.blueprint.pin.revision)throw Error('No confirmed owned enrollment');this.registered=true;}
 ...
 private async persistStop(){
  for(let attempt=0;attempt<32;attempt++){
   ...
   if(!p||p.network!==this.blueprint.network||p.identitySchema!=='provider-index-v1'||p.endpoint!==this.blueprint.pin.endpoint||p.owner!==this.blueprint.pin.owner||p.supervisedRun!==this.runPk||p.supervisedConfigHash!==this.configHash)throw Error('Stop ownership lost');
   if(c&&(c.network!==this.blueprint.network||c.identitySchema!=='provider-index-v1'||c.endpoint!==this.blueprint.subset.endpoint||c.owner!==this.blueprint.subset.owner||c.supervisedRun!==this.runPk||c.supervisedConfigHash!==this.configHash||c.parentScope!==this.blueprint.pin.parent))throw Error('Foreign child cannot be stopped');
