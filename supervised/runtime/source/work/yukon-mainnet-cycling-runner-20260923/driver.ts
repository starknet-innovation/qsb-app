...
import type {Plan} from '../yukon-indexed-controller-20260923/scheduler';
...
export type DriverOptions={maxOperations:number;deadlineMs:number;pollIntervalMs:number;submissionCutoffMs?:number;signal?:AbortSignal};
export async function drive(c:DriverController,eventFor:(stage:string,attempt:number)=>Promise<unknown>,options:DriverOptions){
 ...
 const cutoff=options.submissionCutoffMs??deadlineMs;
 if(!Number.isSafeInteger(cutoff)||cutoff>deadlineMs||cutoff<0)throw Error('Invalid submission cutoff');
 const noNewWork=()=>Date.now()>=cutoff||performance.now()-mono>=cutoff-start;
 ...
 while(operations<maxOperations){
  ...
  if(['reserve','submit'].includes(p.action)&&noNewWork())return summary('submission_cutoff',p);
  ...
  if(p.action==='reserve'){
   ...
   if(signal?.aborted)return summary('aborted',p);
   if(expired())return summary('deadline',p);
   if(noNewWork())return summary('submission_cutoff',p);
