...
export type Config={parent:string;owner:string;revision:number;endpoint:string;deadlineMs:number;submissionCutoffMs:number;maxSubmissions:number};
...
export class SupervisedPin {
 ...
 constructor(private store:Store,enrollment:Enrollment,private provider:ProviderIO,private observe:()=>Promise<Observation>,private emit:(event:Event)=>void,private preserveReceipt:(event:Event)=>Promise<void>,common:{runPk:string;configHash:string},private cpu:Cpu){
  ...
  if(!Number.isSafeInteger(c.submissionCutoffMs)||c.submissionCutoffMs<0||c.submissionCutoffMs>c.deadlineMs)throw Error('Invalid enrolled submission cutoff');
 ...
 private cutoff(){return Date.now()>=this.config.submissionCutoffMs||performance.now()-this.monoStart>=this.config.submissionCutoffMs-this.clockStart;}
 ...
 private budget(row:Row){const b=row.budget as any;if(!b||b.submissionCutoffMs!==this.config.submissionCutoffMs||b.deadlineMs!==this.config.deadlineMs||b.maxSubmissions!==this.config.maxSubmissions||b.maxConcurrent!==1||this.expired()||this.cutoff())throw Error('Enrolled dispatch budget expired or changed');}
 ...
 private async send(intent:string,endpoint:string,event:unknown){
  ...
  if(this.stopping||this.expired()||this.cutoff()){await this.durableOutcome(requestId,intent,'not_sent');throw Error('Stopped before transport entry');}
  ...
  if(this.stopping||this.expired()||this.cutoff()){await this.durableOutcome(requestId,intent,'not_sent');throw Error('Expired before transport entry');}
 ...
 async operation(operation:string,intent:string){
  ...
  if(['reserve','submit'].includes(operation)&&this.cutoff())throw Error('Submission cutoff');
  ...
  const observe=async()=>{if(this.stopping)throw Error('Dispatch stopped');const o=await this.observe();if(this.stopping)throw Error('Dispatch stopped');if(this.cutoff())throw Error('Submission cutoff after preflight');return o;};
