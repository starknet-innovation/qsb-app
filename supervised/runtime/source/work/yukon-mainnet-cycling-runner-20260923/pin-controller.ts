...
export class PinControllerV3 {
 ...
 async initialize(context:any,budget:{maxSubmissions:number;deadlineMs:number;submissionCutoffMs:number}){
  const frozen=JSON.stringify(context);if(!frozen||Buffer.byteLength(frozen)>200000||!['regtest','mainnet'].includes(context.network)||Object.keys(context).sort().join(',')!=='manifest,network,publicStateJson'||['historical-spent-fixture-0','historical-spent-fixture-1','historical-spent-fixture-2'].some(x=>frozen.toLowerCase().includes(x)))throw Error('Fresh public supported-chain context required');
  ...
  await this.store.put({pk:this.pk,sk:'SCOPE',version:1,owner:this.owner,revision:this.revision,endpoint:this.provider.endpoint,identitySchema:SCHEMA,network:ctx.network,stage:'pinning',phase:'pinning_searching',publicContext:frozen,publicContextHash,pinParameters:parameters,budget:{...budget,claimed:0,maxConcurrent:1}});
 }
 async reserve(attempt:number){const s=await this.state();if(attempt<Number(s.pinResumeFloor??0))throw Error('Retired pin coverage cannot be replayed');const ctx=JSON.parse(s.publicContext as string);const req=pinRequest(attempt,createHash('sha256').update(JSON.stringify(ctx.manifest)).digest('hex'),s.pinParameters as any);await this.intents.reserve(attempt,req,async(_,current)=>{if(current.version!==s.version)throw Error('Context changed');});}
 ...
 async poll(sk:string){
  ...
  if(checked.pins.length){
   ...
   const v=cpu.verdict;if(s.excludedPin&&isDeepStrictEqual(s.excludedPin,{sequence:v?.sequence,locktime:v?.locktime}))throw Error('Exhausted pin cannot be reused');
