...
import {createFixedCommon} from '../yukon-common-census-fence-20260923/fixed-common';
import type {CensusStore} from '../yukon-common-census-fence-20260923/census-store';
import type {Blueprint} from '../yukon-common-census-fence-20260923/common';
...
export function validateConfig(input:Config){
 const c=structuredClone(input);exact(c,['format','blueprint','pin','subset']);if(c.format!=='qsb-common-operational-v1'||c.blueprint?.format!=='qsb-common-lifetime-v1')throw Error('Unexpected format');
 for(const [stage,digest] of [['pin',PIN],['subset',SUBSET]] as const){const x=c[stage];exact(x,['id','createdAt','image','socket']);if(!/^[a-z0-9]+$/.test(x.id)||forbidden.includes(x.id)||x.id!==c.blueprint[stage].endpoint||!Number.isFinite(Date.parse(x.createdAt))||x.image!==REPO+'@'+digest||!/^\/[a-zA-Z0-9_./-]+$/.test(x.socket)||x.socket.includes('/../'))throw Error('Endpoint, image or watchdog binding differs');}
