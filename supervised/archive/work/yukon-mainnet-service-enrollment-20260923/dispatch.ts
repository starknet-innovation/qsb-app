...
import {canonicalReservationWrites} from '../../../../server/runtime/storage-authority';
...
export async function createExplicitJob(store:Store,owner:string,body:unknown,validate:PublicValidator){
 const authority=await generationAuthority(store),authorityGeneration=Number(authority.generation);
 ...
 if(existing){const job=existing.job as any;if(job.reservationAuthorityGeneration!==authorityGeneration)throw Error('Job belongs to a different reservation authority');if(job.mainnetRequestHash!==mainnetRequestHash||job.manifestHash!==manifestHash||fingerprint(job.execution)!==fingerprint(execution))throw Error('Idempotency key belongs to another withdrawal or route');routeStoredJob(job,vault);return {created:false,job};}
...
export async function dispatchExplicitJob(store:Store,owner:string,jobId:string,config:Config,launch:Launcher){
 ...
 if(job.reservationAuthorityGeneration!==authorityGeneration)throw Error('Reservation authority changed');
