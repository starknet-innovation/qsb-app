...
No coordinator code change is needed. The strict v2 descriptor identifies the
qsb-solver repository commit, immutable GHCR image digest, generator protocol and
...
Before operating a selected release, configure the Runpod endpoint to its exact
image digest and verify the release provenance and published range vectors. This
is deployment configuration, separate from registering an app descriptor. The
current coordinator uses one configured endpoint; its echoed kernel commit is a
consistency check, **not image attestation**. A descriptor does not deploy or
switch that endpoint. Do not serve incompatible solver selections on one endpoint.
Neither registration nor these tests enables mainnet or proves end-to-end mining.
