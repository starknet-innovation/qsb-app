        ...
        for rep in range(3):
            for count in COUNTS:
                for solver in (('baseline','candidate') if rep%2==0 else ('candidate','baseline')):
                    ...
                    if solver=='candidate':cmd+=['2147483648','1','500000000',str(count)]
                    else:cmd+=['1','0','single_hash','seq_start=2147483648','seq_count=1','lt_start=500000000',f'lt_count={count}']
                    start=time.monotonic();r=subprocess.run(cmd,cwd=cwd,env=env,capture_output=True,text=True,timeout=120);elapsed=time.monotonic()-start
                    ...
                    if solver=='candidate':
                        if r.stdout.splitlines().count(f'QSB_RANGE_DRAINED candidates={count}')!=1:raise ValueError('Missing candidate drain count')
                    elif not re.search(r'\bDone: '+str(count//1000000)+r'M\b',r.stdout):raise ValueError('Historical baseline completion disagrees')
                    records.append({'layout':layout,'rep':rep,'count':count,'solver':solver,'wallSeconds':elapsed,'parameterSha256':request['parameterSha256'],
                                    'hitFiles':{p.name:p.read_text() for p in (cwd/'results').glob('*hit*.txt')}})
                    (out/'partial.json').write_text(json.dumps(records,indent=2)+'\n')
    (out/'results.json').write_text(json.dumps({'binaryHashes':HASHES,'runs':records,'summary':summarize(records),'freshWithdrawal':False,'scope':'interleaved same-GPU unfunded public full-script contexts; process wall-time slopes'},indent=2)+'\n')

if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--inputs',type=Path,required=True);p.add_argument('--out',type=Path,required=True);a=p.parse_args();execute(a.inputs.resolve(),a.out.resolve())
