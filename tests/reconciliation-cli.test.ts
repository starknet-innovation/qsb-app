...
vi.mock("@aws-sdk/client-sfn", () => ({
  StartExecutionCommand: class {
    constructor(
      public input: { name: string; input: string; stateMachineArn: string },
    ) {}
  },
  SFNClient: class {
    send = mocks.workflow;
...
const required = {
  ...
  AWS_BATCH_JOB_BUCKET: "qsb-gpu-jobs",
  AWS_BATCH_JOB_QUEUE: "arn:aws:batch:eu-west-1:123456789012:job-queue/qsb-gpu",
...
it.each(
  ...
  (name, reason) => {
    ...
    const result = spawnSync(
      process.execPath,
      ["--import", "tsx", "scripts/reconcile-submission.ts", ...args],
...
it("reports invalid network before module initialization", () => {
  const result = spawnSync(
    ...
    ["--import", "tsx", "scripts/reconcile-submission.ts", ...args],
    {
      env: { PATH: process.env.PATH, ...required, QSB_NETWORK: "invalid" },
      encoding: "utf8",
      timeout: 10000,
    },
  );
...
it("refuses a disabled polling route before credentials or storage reads", async () => {
  ...
  const get = vi.spyOn(store, "get"),
    secretCalls = mocks.secret.mock.calls.length;
  const stderr = vi.spyOn(process.stderr, "write").mockReturnValue(true);
  await reconcileSubmissionCli(args);
  expect(process.exitCode).toBe(1);
  expect(stderr).toHaveBeenCalledWith(
    expect.stringContaining("PollingNotAllowed"),
  );
...
it("prints the polling refusal reason, exits nonzero and preserves the attached ID", async () => {
  ...
  mocks.secret.mockResolvedValue({
    SecretString: JSON.stringify({ apiKey: "public-test-placeholder" }),
  });
  mocks.workflow.mockRejectedValue(
    Object.assign(new Error(), { name: "ExecutionAlreadyExists" }),
  );
  vi.spyOn(AwsBatch.prototype, "status").mockResolvedValue({
    id: "provider-1",
    status: "IN_PROGRESS",
  });
  ...
  expect(JSON.parse(stdout.mock.calls[0][0] as string)).toMatchObject({
    ...
    reason: "execution-already-exists",
    providerId: "provider-1",
  });
  expect((await store.get("OWNER#owner", "JOB#job"))?.job).toMatchObject({
    runpodId: "provider-1",
    status: "searching",
  });
});
...
it.each(["400", "499"])(
  "CLI records immediate HTTP %s recovery without waiting TTL",
  async (status) => {
    for (const [key, value] of Object.entries(required)) vi.stubEnv(key, value);
    const owner = `http-${status}`,
      pk = `OWNER#${owner}`;
    const job = {
      batchSubmission: {
        jobName: "qsb-test",
    ...
    vi.spyOn(AwsBatch.prototype, "health").mockResolvedValue({
      jobs: { inQueue: 0, inProgress: 0 },
      workers: {},
    } as any);
    vi.spyOn(process.stdout, "write").mockReturnValue(true);
    vi.spyOn(process.stderr, "write").mockReturnValue(true);
    await reconcileSubmissionCli([
      owner,
      "job",
      "--not-submitted",
    ...
    expect((await store.list(pk, "RECONCILIATION#"))[0]!.decision).toEqual(
      stored.submissionReconciliation,
    );
  },
);
it.each([undefined, "399", "500", "timeout", "400.5"])(
  "CLI refuses invalid rejection status %s before credential access",
  async (status) => {
    for (const [key, value] of Object.entries(required)) vi.stubEnv(key, value);
...
it.each([undefined, "", " ", "TRUE", "1", "false ", " true", "enabled"])(
  ...
  (value) => {
    ...
    expect(JSON.parse(result.stderr)).toEqual({
      action: "refuse",
      reason: value?.trim()
        ? "QsbMainnetEnabledInvalid"
        : "QsbMainnetEnabledRequired",
    });
    expect(result.stdout).toBe("");
  },
);
it.each(["true", "false"])(
  "accepts an explicit mainnet switch %s for later route checks",
  (value) => {
    expect(
...
it("restarts a recorded paid job after its STANDARD execution closes under a new audited name", async () => {
  ...
  const job = {
    ...
    batchSubmission: {
      ...
      inputSha256: "a".repeat(64),
      inputKey: "inputs/test.json",
      queue: required.AWS_BATCH_JOB_QUEUE,
      definition: required.AWS_BATCH_JOB_DEFINITION,
    },
    stage: "pinning",
    attempt: 7,
    revision: 3,
    computeSeconds: 25,
    gpuBudgetReservedSeconds: 120,
    gpuSubmissions: 1,
    ...
    retryRequested: true,
    error: "provider read unavailable",
  } as Job;
  ...
  mocks.workflow.mockImplementation(async (command) => {
    if (closedNames.has(command.input.name))
      throw Object.assign(new Error("closed execution"), {
  ...
  const prepare = vi
    .spyOn(AwsBatch.prototype, "prepareRun")
    .mockRejectedValue(new Error("must not prepare paid work"));
  const cancel = vi
    .spyOn(AwsBatch.prototype, "cancel")
    .mockRejectedValue(new Error("must not cancel"));
  const stdout = vi.spyOn(process.stdout, "write").mockReturnValue(true);
  vi.spyOn(process.stderr, "write").mockReturnValue(true);
  ...
  expect(starts).toEqual([
    {
      name: "recorded-job-r4",
      stateMachineArn: required.WORKFLOW_ARN,
  ...
  expect(
    (await store.get(pk, "RECONCILIATION#recorded-job#3"))?.decision,
  ).toEqual(stored.submissionReconciliation);
