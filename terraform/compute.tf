...
resource "aws_lambda_function" "coordinator" {
  ...
  reserved_concurrent_executions = var.lambda_concurrency
  environment {
    variables = merge({ TABLE_NAME = aws_dynamodb_table.records.name, QSB_NETWORK = var.network, SOLVER_RELEASE_ID = local.solver_release_id, QSB_MAINNET_ENABLED = tostring(var.mainnet_enabled), QSB_REHEARSAL_ENABLED = "false", REFERENCE_FUNCTION = aws_lambda_function.reference.function_name }, local.gpu_limit_env, local.compute ? { AWS_BATCH_JOB_QUEUE = var.batch_job_queue, AWS_BATCH_JOB_DEFINITION = var.batch_job_definition, AWS_BATCH_JOB_BUCKET = var.batch_job_bucket } : {})
  }
