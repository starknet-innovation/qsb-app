...
resource "aws_s3_object" "dispatcher" {
  for_each               = var.provision_runtime ? toset(["dispatcher.cjs", "host.py", "manifest.json", "install.py", "credential-exec.py", "watchdog.mjs", "watchdog-entry.mjs", "qsb-dispatch.service", "qsb-watchdog@.service"]) : toset([])
  bucket                 = aws_s3_bucket.runtime[0].id
