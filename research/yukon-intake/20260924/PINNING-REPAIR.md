...
## Release-fenced research submission composition

`submitResearchPin` now composes actual public CPU preflight, the existing indexed
intent inventory, and the exact queue envelope. It requires explicit research
execution authorization and startup-capacity validation in trusted enrollment;
receive-only enrollment does not authorize spending. No live enrollment is
created here, and the provider startup-capacity issue remains unresolved.

Reservation, budget claim and final dispatch fence all condition on the selected
release version. Scope endpoint/context/release binding must remain unchanged.
The provider POST is called at most once for that intent; transport exceptions
retain uncertainty. Identity journaling is deliberately not blocked by later
release revocation, so a returned late provider ID remains recoverable. As with
any database/provider composition, revocation cannot atomically retract a POST
already authorized and in flight. The trusted transport and provider preflight
adapters are not public request callbacks.

The CPU bridge gains a checked `prepare` action; its local source lock was updated.
The GPU package regenerated byte-for-byte identically to the tested queue package.
Six new tests cover racing claims, revocation in provider preflight, unknown POST
outcomes without retry, late IDs after revocation, invalid public parameters
before reservation, and separate execution/capacity authorization. All **27**
focused TypeScript tests and typecheck pass; all **24** Store tests pass against
actual loopback DynamoDB Local with real CPU processes and mocked paid transport.
`runtime/submission-route/dynamodb-local.json` records source hashes and cases.
All test tables and the container were removed; there were no live provider calls.
