...
- Final integrated source/image identity, deployment isolation, and permission review. A source manifest, in-process handoff, local host rehearsal, and local storage-cutover rehearsal now exist in this checkout. No production host is selected. The public-build worker, CPU-reference, supervisor, and local OCI layout identities from `4763c70` are enrolled and are not this tree's executable or a pushed registry manifest. Regional IAM and a fresh CPU-verified search do not. A source review of the ranked generic field path and a pin-scoped coverage ledger are now checked in this checkout. They are not a GPU rerun. In-memory coverage does not measure the HOLD solver binary recorded by `docs/source-build/20260924/solver-build-receipt.json`.
...
The [24 September clean-checkout source-build report](source-build/20260924/REPORT.md) records successful native CUDA, queue-image and reproducible supervisor builds, with exact output identities. Those identities are enrolled for that build. Later source edits, a pushed registry manifest, and fresh execution remain open.
