...
| Public checkout | `vitest run` in this checkout reported 29 files and 219 tests, and `tsc --noEmit` passed, after pinned dependency preparation. The initial public export recorded 134. | Browser/GPU tests, deployment, or a fresh withdrawal reproduced from this public checkout. |
...
- [x] Confirm complete range partitioning and coverage accounting across batches, retries, pin changes, both subset rounds and session replacement. Coverage from a prior pin must not transfer to a new pin. Evidence: `server/runtime/coverage-ledger.ts`. Pinning attempts `0..134217727` cover sequences `[2^31, 2^32)`. Round one and round two share attempts `0..4828` and end at C(150, 9); the test walks every subset boundary. A ledger retry does not credit. A completed attempt credits once. A reconciled terminal provider result is submitted again after resume and is not skipped; an uncertain submission with no id is not replayed. A stopped coverage account is failed and cannot be resumed. Exhaustion is refused while the current stage is absent from the ledger. Round credit is stored under `sequence:locktime`. Another search pin, session id, or solver pin reads an empty account, and `replaceSession` does not copy. When eight accounts are already stored, `replaceSession` returns `coverage-account-full` and does not look like a new empty session. Both rounds can be fully accounted while `wholeRangeCovered` stays false. Every credit names the recorded HOLD solver binary and sets `measuresHoldSolverBinary` false. `validationTick` keeps an exhausted pin's round credit off the next pin. The stored form is merged intervals. More than 64 fragments, 64 search pins, or 8 accounts stops without credit. A full contiguous sweep is one interval; the cap is not a production encoding of a sweep that stays fragmented.
