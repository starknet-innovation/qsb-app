...
| Public checkout | `vitest run` in this checkout reported 28 files and 238 tests, and `tsc --noEmit` passed, after pinned dependency preparation. The parent checkout `1c34f60` reported 26 files and 217 tests. The initial public export recorded 134. | Browser/GPU tests, deployment, or a fresh withdrawal reproduced from this public checkout. |
...
- [x] Review the arithmetic assumptions and admitted representations for the exact selected generic path, including guarded field helpers, normalization, inversion and point recovery. Record remaining algorithm assumptions explicitly. Evidence: `reviewGenericPath` in `server/runtime/solver-review.ts` and `worker/validation/field_assumptions.py`, asserted by `tests/solver-coverage.test.ts`. The ranked generic path is the enum kernel at `ZLAB_TRIM=0` and `QSB_PAIR_SHARED=0`. Admitted field words are four little-endian limbs below 2^256. One conditional subtraction canonicalizes them because 2p is greater than 2^256; the CPU check compared that rule with reduction modulo p on boundary and random words. Strict `qsb_exact_der` matched `is_valid_der` on 32-byte vectors. `algorithmAssumptions` records what remains: no universal divstep bound, no PTX execution, the pair filter can omit a candidate before verification, and finite vectors are not universal correctness. `nativeBinarySha256` stays null and `gpuExecuted` stays false.
- [x] Confirm complete range partitioning and coverage accounting across batches, retries, pin changes, both subset rounds and session replacement. Coverage from a prior pin must not transfer to a new pin. Evidence: `server/runtime/coverage-ledger.ts`. Pinning attempts `0..134217727` cover sequences `[2^31, 2^32)`. Round one and round two share attempts `0..4828` and end at C(150, 9); the test walks every subset boundary. A retry does not credit. A completed attempt credits once. Round credit is stored under `sequence:locktime`. Another search pin, session id, or solver pin reads an empty account, and `replaceSession` does not copy. Both rounds can be fully accounted while `wholeRangeCovered` stays false. `validationTick` keeps an exhausted pin's round credit off the next pin. The stored form is merged intervals. More than 64 fragments, 64 search pins, or 8 accounts stops without credit. A full contiguous sweep is one interval; the cap is not a production encoding of a sweep that stays fragmented.
...
- [x] Ensure deterministic failures and unsupported geometry stop without range credit. The current bounded hit output fails closed beyond its supported capacity; it is not unlimited output support. Evidence: the ledger refuses deterministic failure, unsupported geometry, host error, publication failure, an unresolved exceptional denominator, and a hit count above 64, and then refuses later credit on that account. `validationTick` credits a `range-complete` batch that contains a CPU-valid hit before changing stage, and pauses instead when that checkpoint is incomplete. It also pauses a publication with more than 64 `indices=` or `sequence=` records, does not call the CPU verifier for that overflow, and does not increment `completed`. Optimized host paths in `research/optimized-subset` return `QSB_RANGE_INCOMPLETE` before publishing a truncated list. The enrolled `worker/prepare_kernels.py` bytes are unchanged, so the Dockerfile image still truncates at 64 hits and returns success; that image must not inherit this result. `tests/solver-coverage.test.ts` checks that the prepared historical sources still contain `? 64 :` and still lack host exceptional recovery, while the optimized tree has no truncation ternary. This was not executed on a GPU. A kernel that returns success after skipping an unusable point is still the open gap in 5.3, and candidate checks cannot see that omission.
...
- **5.5** `judgeEvidence` rejects a predecessor GPU claim and rejects a native hash while `nativeBinarySha256` is null. The CUDA audits in this tree were not rerun. `docs/gpu-validation` is absent. Do not attach an earlier measurement to these bytes.
