...
- `callMinerSubmit` and `Slipstream.submit` check that permit before the transport. A missing or cloned permit throws `SpendAuthorizationRequired` and does not call the transport. A mainnet permit, or any permit aimed at the mainnet miner host, throws `MainnetTransportRefused` before fetch. `callMinerSubmit` accepts only a sealed `localMinerTransport` double. A function, a forged local object, or an endpoint on `slipstream.mara.com` or `teststream.mara.com` throws `LiveMinerTransportRefused` and is not invoked. The vault fund and withdrawal submit routes refuse mainnet before miner preflight or a saved intent, and they do not pass the live miner into `callMinerSubmit`. A local double result keeps `included` false. This checkout does not call a live miner.
...
6. Only a sealed local miner double may be invoked by `callMinerSubmit`. A function or a live `slipstream.mara.com` / `teststream.mara.com` endpoint is refused before invocation. A mainnet permit is refused before any miner HTTP, including preflight, and `Slipstream.submit` also refuses the mainnet miner host. A missing permit is rejected before any miner HTTP. Do not point this checkout at a live miner. A success body from the miner is not inclusion.
