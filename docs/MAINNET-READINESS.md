...
7. After a real broadcast made under a separate authorization, confirm the exact transaction from chain data. Preserve the block hash, block height, and transaction id. `judgeInclusionEvidence` can mark that record structurally complete, and `independentlyConfirmed` stays false until the status route's own Esplora query returns it. HTTP 200, a mempool preflight, and a miner-reported confirmation are not inclusion. Bring the evidence back for review. `section7Closed` stays false until that review records it. This checkout has no such evidence.
