...
- [x] Package the complete reviewed API, dispatcher, runtime, CPU verifier and evidence-reader dependency closure without reads from a developer work directory. Evidence: `release/source-manifest.json`, rebuilt by `npm run package:release` after `npm run vendor`. `tests/release-package.test.ts` copies that closure into a temp tree and rechecks hashes. Paths outside this checkout are rejected. This is a source closure, not an OCI image.
...
- [x] Recheck wrong-chain rejection, capability revocation, duplicate requests, wallet changes and existing-job guards in the final composition. Evidence: `tests/runtime-handoff.test.ts` covers a `testnet4` service guard, a non-mainnet request body, a different wallet session, an idempotency conflict, a second job for the same vault, and `enabled: false` on the capability row.
