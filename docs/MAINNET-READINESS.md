...
| Public checkout | `vitest run` in this checkout reported 29 files and 264 tests, and `tsc --noEmit` passed, after pinned dependency preparation. The parent checkout `e018aa7` reported 28 files and 238 tests. The initial public export recorded 134. | Browser/GPU tests, deployment, or a fresh withdrawal reproduced from this public checkout. |
...
- `judgeCoreReport` and `admitCoreHarnessResult` refuse a report that sets a fresh-withdrawal flag, sets `section6Closed`, or names mainnet. A puzzle-relaxed regtest report stays `section6Closed: false`. Both the shell harness and admission load `server/runtime/core-binary.json`. This checkout enrolls none (`enrolled: false`, both hashes null), so a file merely named `bitcoind` is not harness evidence and the script exits 2. A hash-matched report is admitted only when that same file enrolls the hashes. The live harness records those executable digests on `coreBinaries`. `tests/core_regtest.py` writes the not-run report before importing vendor modules. The packaged `package.json` omits `harness:core` because the launcher and its vendor imports are outside the release closure. The puzzle-relaxed spend bypasses three puzzle checks and is not this gate.
