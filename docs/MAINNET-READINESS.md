...
6. Only a granted non-mainnet permit may reach a miner transport from this checkout. A mainnet permit is refused before any miner HTTP, including preflight, and `Slipstream.submit` also refuses the mainnet miner host. A missing permit is rejected before any miner HTTP. Do not point this checkout at a live miner. A success body from the miner is not inclusion.
