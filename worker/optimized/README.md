...
The apt package repositories are not snapshot-locked, and Python package versions are locked without wheel content hashes, so this is a source-complete build recipe, **not a claim of universal bit-for-bit container reproducibility**. Record the generated receipt and OCI digest for each release. Historical speed/correctness evidence does not automatically certify the new binary. Python wheel content hashes and OS snapshotting are further reproducibility improvements.
