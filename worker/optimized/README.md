...
The apt package repositories and SDK transitive dependencies are not fully snapshot-locked, so this is a source-complete build recipe, **not a claim of universal bit-for-bit container reproducibility**. Record the generated receipt and OCI digest for each release. Historical speed/correctness evidence does not automatically certify the new binary. SDK transitive locking and OS snapshotting are further reproducibility improvements.
