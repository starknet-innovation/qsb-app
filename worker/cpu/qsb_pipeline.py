...
from secp256k1 import (
    ...
    encode_der_sig, is_valid_der_sig, is_recoverable_der_sig, modinv, int_to_der_int,
)
...
def puzzle_hash(pubkey_bytes, hash_mode='ripemd160', hash_choice=0):
    ...
    if hash_mode == 'ripemd160':
        ...
        return h, is_recoverable_der_sig(h), 0
        ...
        if is_recoverable_der_sig(h1):
            return h1, True, 0
        # Try SHA-256(SHA-256) (bit=1 — IF branch taken)
        h2 = hashlib.sha256(h1).digest()
        if is_recoverable_der_sig(h2):
            return h2, True, 1
...
def cmd_export(args):
    ...
    # QSB app patch: reproduce build_full_script's shared stack model.
